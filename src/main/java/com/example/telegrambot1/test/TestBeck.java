...
public class TestBeck implements Test{
    ...
    @Override
    public boolean needPsychiatrist() {
        return result >= 10;
    }
    ...
    public String getTextResult() {
        String resultText = "Ваши баллы: " + result + ".\n";
        if (result>=30) {
            resultText += "Симптомы тяжелой депрессии. Это последняя, самая опасная стадия депрессии, с которой не получится справиться в одиночку. Необходимо в срочном порядке обратиться за помощью к психотерапевту.";
        } else if (result>=20) {
            resultText += "Признаки выраженной депрессии (средней тяжести). Депрессия на этой стадии создает преграды для нормальной жизни и легко может перейти в тяжелую депрессию. Такое состояние нельзя оставлять без внимания и медицинского контроля, необходимо в обязательном порядке обратиться за консультацией к психотерапевту.";
        } else if (result>=16) {
            resultText += "Симптомы умеренной депрессии. Депрессия на данном этапе создает определенные трудности для нормальной жизни и опасна быстрым переходом на следующую стадию. Необходимо обратиться за помощью к психотерапевту.";
        } else if (result>=10) {
            resultText += "Признаки легкой депрессии (субдепрессии). Пора задуматься о воем психическом здоровье и обратиться за консультацией к психотерапевту.";
        } else {
            resultText += "Симптомы депрессии отсутствуют, Ваше психоэмоциональное состояние в норме и не вызывает никаких опасений.";
        }
        return resultText;
    }
